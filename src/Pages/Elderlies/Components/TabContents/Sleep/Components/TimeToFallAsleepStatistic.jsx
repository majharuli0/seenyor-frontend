...
export default function TimeToFallAsleepStatistic({ isSummaryBtn = true }) {
  const { title, color, icon, dataAnalysis, summaryProps, description } =
    getTemplateData('Time to Fall Asleep');
  ...
  const [timeToFallAsleepStatistic, setTimeToFallAsleepStatistic] = useState([]);
  const getSleepDurationDistributionData = useCallback(() => {
    ...
    getFallAsleepStatistics({
      ...
      .then((res) => {
        // setTimeToFallAsleepStatistic(res.data);
  ...
  function filteredData(data) {
    ...
    setTimeToFallAsleepStatistic(filteredData);
  }
  useEffect(() => {
  ...
  return (
    ...
    >
      <BarChart
        data={timeToFallAsleepStatistic}
        color={color}
        dataType='duration'
        chartFor='timeToFallAsleep'
        toDate={toDate}
        fromDate={fromDate}
      />
    </Template>
