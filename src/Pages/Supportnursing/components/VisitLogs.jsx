...
import { Table, Select, DatePicker, Spin, Button, ConfigProvider, Input } from 'antd';
...
const VisitLogsTable = () => {
  ...
  const columns = [
    ...
    {
      title: <span className='px-0 block text-nowrap font-medium text-gray-700'>Room #</span>,
      key: 'room_no',
      render: (row) => (
        <div className='px-2'>
  ...
  return (
    <div className='p-4 bg-white rounded-[12px] w-full'>
      <h2 className='text-[22px] font-semibold mb-4'>Visit Logs</h2>
      <ConfigProvider theme={{ token: { colorPrimary: '#64748b' } }}>
        <div className='flex items-center gap-3 mb-4'>
          ...
          <div
            className='flex items-center px-3 py-1 border rounded cursor-pointer text-nowrap'
            onClick={() => setSortOrder((prev) => (prev === 1 ? 0 : 1))}
          >
            {sortOrder === 1 ? 'Oldest First' : 'Latest First'}
            {sortOrder === 1 ? (
              <ArrowDownOutlined classID='ml-1' />
            ) : (
              <ArrowUpOutlined classID='ml-1' />
            )}
          </div>
